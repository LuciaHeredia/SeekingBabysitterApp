package com.example.seekingbabysitter.validator
...
class Validator {
    ...
    companion object {
         ...
         * @return - true if the phone is valid.
         */
        fun isValidAge(data: Any, updateUI: Boolean = true): Boolean {
            val str = getText(data)
            var valid = true

            // Age policy check
            // Age should be 2 characters long
            if (str.length != 2) {
                valid = false
            }
            // Age's first number can't be 0
            var exp = "0.*"
            ...
            // Set error if required
            if (updateUI) {
                val error: String? = if (valid) null else AGE_VALIDATION_MSG
                setError(data, error)
            }
